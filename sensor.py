...
class MeteoSensorTemp(SensorEntity):
    ...
    def update(self) -> None:
        ...
        for meteo in self._meteos:
          status = meteo.status(self._domService)
          _LOGGER.debug(f"Meteo status: {status}")
          celsius = status.getCelsius()
          if (celsius < -20 or celsius > 60):
              _LOGGER.warning(f"Temperature value {celsius}°C for {self._attr_name} is out of expected range. Skipping.")
              continue
          temp += celsius
          validReadings += 1
        if validReadings > 0:
            avgTemp = round(temp / validReadings, 2)
            _LOGGER.debug(f"External temperature: {avgTemp}")
            self._attr_native_value = avgTemp
        else:
            _LOGGER.warning(f"No valid temperature readings for {self._attr_name}")
